...
Docker Compose 是最简单快速的部署方式，适合想要快速体验或生产环境部署的用户。
...
> 📌 **提示**: Electron 客户端包含完整的前后端，无需额外部署服务器。
...
Electron 客户端是独立的桌面应用，**已内置后端服务**，开箱即用：

1. **首次启动**: 双击安装后的应用图标
2. **自动配置**: 应用会自动配置本地后端服务（无需手动设置）
3. **开始使用**: 直接创建文章、配置发布渠道即可

> ✨ **优点**: 无需任何配置，安装即用，适合个人用户。

#### Web 版客户端配置

如果你使用 Docker 或源码部署了后端服务，可以通过浏览器访问：

##### Docker 部署（自动配置）

如果前后端部署在同一域名下（Docker 默认方式），**无需配置**：
...
如果后端部署在其他服务器或端口，需要手动配置：

1. **打开应用**: 在浏览器中访问前端地址
2. **配置服务器**: 首次访问会提示配置后端服务器地址
3. **输入地址**: 填写后端服务器地址（例如：`http://192.168.1.100:3000`）
4. **测试连接**: 点击「测试连接」按钮，确认连接成功
5. **保存配置**: 点击「保存」按钮完成配置

> 💡 **提示**: 后续可在 **设置 → 服务器配置** 中修改服务器地址。
...
A:
- **Electron 客户端**: 独立桌面应用，内置后端服务，安装即用，支持弹窗登录
- **Web 版**: 需要单独部署后端，通过浏览器访问，适合服务器部署和多人使用

**Q: Web 版如何连接后端服务？**

A:
1. 如果使用 Docker 部署，前后端在同一域名下，会自动检测配置
